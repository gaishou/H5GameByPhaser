var playState = {
    ...
    preload:function () {
        ...
        game.load.tilemap(this.map,null,map_obj,
    ...
    onMenu:function () {
        ...
        this.return_button = new ButtonPrefab(cx,cy+96,'return',
            playState.resumeCallback,this);
    ...
    showReadyGo:function () {
        ...
        this.ready_label = game.add.text(cx,cy,'READY',
            {font:'64px Arial',fill:'#fff',stroke:'#a50',strokeThickness:10});
    ...
    this.result_label = game.add.text(cx,cy,text,{font:'72px Arial',
    fill:'#fff',stroke:'#850',strokeThickness:10});
    this.result_label.anchor.setTo(0.5,0.5);
    this.result_label.scale.set(0.1,0.1);
    var tween = game.add.tween(this.result_label.scale).to
    ({x:1,y:1},1500,Phaser.Easing.Quartic.InOut,true);
    panel.status = 'ending';
