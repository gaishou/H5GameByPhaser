...
Character.prototype.walk = function (direction){
    ...
    //判断是否到边界或障碍物
    if(gx< 0 || gx>= this.panel.width||gy <0 ||gy>= this.panel.height ||!this.panel.walkable(gx,gy)){
        this.standStill();
        return false;
    }
    this.goal_mapx=gx;
...
Character.prototype.putBomb() = function () {
    if(panel.canPutBomb(this.mapx,this.mapy)){
        var bomb =new Bomb(panel,this.mapy, this.attack, this.bomb_type, this.bomb_range, this.bomb_shape);
        panel.putBomb(bomb);
        return true;
    }
    return false;
