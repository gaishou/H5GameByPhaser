var levelState = {
    ...
    levelCallback :function (i) {
        ...
        this.camera.onFadeComple.addOnce(function () {
    ...
    addButton:function (i,type) {
        ...
        var b =null;
        if(type =='passed')
            b = game.add.button(e.iconx,e.icony,'image',
                ()=>this.levelCallback(i),this,'system/system_06', 'system/system_05', 'system/system_07');
        else if(type == 'next')
            b = game.add.button(e.iconx,e.icony,'image',()=> this.levelCallback(i), this,
                'system/system_02', 'system/system_01', 'system/system_03');
        else
    ...
    create:function () {
        ...
        this.level_background = game.add.sprite(0,0,'game','level');
        ...
        for (let i in this.levels){
            if(is_passed[i])
                continue;
            if(is_next[i]){
                this.addButton(i,'next');//当玩家通关后解锁下一关卡
            }else {
                this.addButton(i,'unlocked');//当关卡未通过，下一关的关卡按钮不能点击
            }
        }
        if(passed_count == 0)
            this.addButton(0,'next');
        //添加返回按钮
        this.return_button = new ButtonPrefab(game.world.
        width-16,game.world.height-16,'return',levelState.
        returnCallback,this);
